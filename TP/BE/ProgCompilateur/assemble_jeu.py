...
for l in asm:
    ...
    if(len(l_wo_comments) != 0  and l_wo_comments[-1] == ':'):
        print("[Erreur] une étiquette ne peut pas apparaître seule sur une ligne, elle doit forcément être suivie d'une instruction, %s ligne %i" % (asm_filename, asm_lnumber))
        sys.exit(-1)
    ...
    if(len(fields) > 1):
        ...
        try:
            ...
            print "[Erreur] une étiquette ne doit pas être interprétable comme une valeur hexadécimale; l'étiquette \"%s\" peut l'être;  %s ligne %i" % (fields[0].strip(), asm_filename, asm_lnumber)
            sys.exit(-1)
        except ValueError:
            pass
    ...
    if(inst_name == 'DSW'):
        if(not inst_op):
            print "[ERREUR] l'instruction %s attends un nom de variable, %s ligne %i" % ('DSW', asm_filename, asm_lnumber)
            sys.exit(-1)
        if(not header_code):
            print "[ERREUR] les instructions DSW doivent se trouver en tête du programme, après éventuellement des JMP, %s ligne %i" % (asm_filename, asm_lnumber)
            sys.exit(-1)
        ...
        try:
            ...
            print "[Erreur] un nom de variable ne doit pas être interprétable comme une valeur hexadécimale; la variable \"%s\" peut l'être;  %s ligne %i" % (inst_op, asm_filename, asm_lnumber)
            sys.exit(-1)
        except ValueError:
            pass
    ...
    # Sinon ce doit être une instruction machine
        ...
        if(inst_name not in instructions):
            print "[Erreur] l'instruction %s n'est pas reconnue, %s ligne %i " % (inst_name, asm_filename, asm_lnumber) 
            sys.exit(-1)
        ...
        if((inst_op and expected_n_op == 0) or ((not inst_op) and expected_n_op == 1)):
            if(inst_op and expected_n_op == 0):
                print "[Erreur] l'instruction %s n'attend pas d'argument, %s ligne %i" %(inst_name, asm_filename, asm_lnumber)
                sys.exit(-1)
            else:
                print "[Erreur] l'instruction %s attends un argument, %s ligne %i" %(inst_name, asm_filename, asm_lnumber)
                sys.exit(-1)
...
for l in bytecode.split():
    try:
        ...
        print("[Erreur] la chaîne %s ne corresponds pas un code hexadécimal valide. Avez vous fait référence à des étiquettes ou variables non définies ?" % l)
...
f.write('v2.0 raw\n' + bytecode)
f.close()
print("Le code machine a été généré et sauvegardé dans le fichier %s" % bytecode_filename)
